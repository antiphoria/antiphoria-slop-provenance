...
def test_is_research_use_acknowledged_env(
    ...
    # Isolate from developer machine ~/.config ack file written by slop-cli.
    monkeypatch.setattr(rua, "_config_dir", lambda: tmp_path / "empty_cfg")
...
def test_ack_file_roundtrip(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.delenv("ANTIPHORIA_SLOP_PROVENANCE_RESEARCH_ACK", raising=False)
